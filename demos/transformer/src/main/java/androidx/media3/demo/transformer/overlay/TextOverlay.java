...
public abstract class TextOverlay extends BitmapOverlay {
 ...
 public TextOverlay(int width, int height, int textSize) {
   this.width = width;
   this.height = height;
  ...
  protected @MonotonicNonNull Bitmap lastBitmap;
  protected @MonotonicNonNull SpannableString lastText;
  ...
  public Bitmap getBitmap(long presentationTimeUs) {
    Log.i(TAG, "getBitmap presentationTimeUs=" + presentationTimeUs / C.MILLIS_PER_SECOND);
    ...
    if (!overlayText.equals(lastText)) {
      ...
      Log.w(TAG, "getBitmap draw");
    }
    return checkNotNull(lastBitmap);
  }

  protected int getSpannedTextWidth(SpannableString text, TextPaint textPaint) {
